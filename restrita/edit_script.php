...
<body>
    ...
    <div class="container">
        <div class="row">
            ...
            $sql = "UPDATE tabela_de_gastos SET nome = ?, tipogastos = ?, data = ?,deposito = ?, valor = ? WHERE idpessoa = ?";
            $stmt = mysqli_prepare($conn, $sql);
            // Verifica se a preparação da query foi bem-sucedida.
            if ($stmt) {
                // Vincula os parâmetros à query preparada.
                mysqli_stmt_bind_param($stmt, "sssssi", $nome, $tipogastos, $data,$deposito, $valor, $id);
